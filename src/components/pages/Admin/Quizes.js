...
const Quizes = (props) => {
  ...
  function copyToClipboard(e, duration) {
    inptRef.current.value = `https://reactivequiz.com/start-quiz/${btoa(
      JSON.stringify({
        quiz: "trainer",
        quizId: e.target.id,
      })
  ...
  const sortingBy = (e, field) => {
    ...
    switch (field) {
      ...
      case "duration":
        ...
        setChangedQuizes(
          changedQuizes.sort((a, b) =>
            orderingDuration ? a[field] - b[field] : b[field] - a[field]
          )
        );
        break;
      case "questions":
      ...
      <div className="quizesWrapper">
        ...
        {changedQuizes.length ? (
          <div className="quizesTableDiv">
            <input type="text" ref={inptRef} className="linkToCopyInpt" />
                  ...
                  <tr key={`1${quiz.dbId}`}>
                    ...
                    <td>
                      <div
                        ...
                        onClick={(e) =>
                          filterByQuiz(e, quiz.questions, quiz.name)
                        }
                      ...
                      >
                        {" "}
                        {quiz.name}{" "}
                        {hint && hint.index === quiz.dbId ? (
                          <div className={s.hint}>
                            Click to see quiz questions!
                          </div>
                        ) : null}
                      </div>
                    </td>
                    ...
                    <td>
                      ...
                      >
                        <option key={`1${quiz.dbId}`} value={quiz.status}>
                          {quiz.status}
                        </option>
                        <option key={`2${quiz.dbId}`} value="enabled">
                          enabled
                        </option>
                        <option key={`3${quiz.dbId}`} value="disabled">
                          disabled
                        </option>
                      </select>
                    </td>
