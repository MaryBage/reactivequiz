...
const Questions = (props) => {
  const { deleteData, questions, addQuizes, updateData } = useContext(
    DbContext
  );
  ...
  const [qstnInptValue, setQstnInptValue] = useState(
    questions.reduce(
      (txt, value, key) => ({ ...txt, [`question${key}`]: value.question }),
      {}
    )
  );
  ...
  useEffect(() => {
    checkAll && setSelectedQstns(filteredValue.map((e) => e.questionDbId));
  }, [checkAll]);

  useEffect(() => {
    if (selectedQstns.length > 0 && selectedQstns.length < filteredValue.length)
      setCheckAll(false);
    ...
    <>
      ...
      {props.location.hash && (
        <div className="filterBy">
          Filtered by quiz : {props.location.hash.slice(1)}
        </div>
      )}
      <div className={s.questionWrapper}>
        <div className={s.buttonsWrapper}>
          <button
            className={s.createQuiz}
            name="createQuiz"
            onClick={openModal}
          >
            compose quiz
          </button>
          <button className={s.selectAll} name="selectAll" onClick={selectAll}>
            {checkAll ? "un" : null}select all
          </button>
          <button className={s.deleteAll} name="deleteAll" onClick={deleteAll}>
            delete
          </button>
        </div>
        <hr />
                  ...
                  <li className={s.questionItemLi}>
                    <div className={s.inputWrapper}>
                      <input
                        ...
                        checked={
                          checkAll ||
                          selectedQstns.includes(currentQuestion.questionDbId)
                        }
                      ...
                      <div
                        className={s.tick}
                        onClick={(e) =>
                          makeToShow(e, currentQuestion.questionId)
                        }
                      >
                      ...
                      <div
                        ...
                        onDoubleClick={(e) =>
                          makeToDisappear(e, currentQuestion.questionId)
                        }
                        onMouseOver={(e) =>
                          makeToShowHint(e, currentQuestion.questionId)
                        }
