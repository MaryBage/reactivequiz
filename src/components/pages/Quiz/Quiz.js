...
const Quiz = (props) => {
    ...
    setQuiz(
      quiz.map((questionItem, i) => {
        return {
          ...
          userAnswer:
            i === activeQuestion
              ? singleQuiz.userAnswer
              : questionItem.userAnswer,
          isActive: i === activeQuestion - 1 ? true : false,
          isSubmitted:
            i === activeQuestion
              ? singleQuiz.isSubmitted
              : questionItem.isSubmitted,
        };
      })
    );
  };
        ...
        return {
          ...
          isActive:
            activeQuestion < quiz.length - 1
              ? i === activeQuestion + 1 || activeQuestion + 1 == quiz.length
                ? true
                : false
  ...
  const finishQuiz = () => {
    ...
    axios.post(`/calcResult.php`, btoa(JSON.stringify(params))).then((res) => {
                ...
                {quiz.map((el, i) => {
                  return (
                    <a
                      href="#"
                      className={
                        i == quiz.findIndex((item) => item.isActive)
                          ? "activeQstn"
                          : el.isSubmitted
                          ? "passedQuestion"
                          : null
                    ...
                    >
                      {el.isSubmitted ? <span>&#10004;</span> : null} Question{" "}
                      {i + 1}
                    </a>
                  );
                })}
