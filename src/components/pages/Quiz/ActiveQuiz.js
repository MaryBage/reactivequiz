    ...
    if (quiz.type == "multiple") {
      if (answeredQuiz.userAnswer.includes(+e.target.id))
        ...
        setQuiz({
          ...answeredQuiz,
          isSubmitted: false,
          userAnswer: answeredQuiz.userAnswer.concat(+e.target.id),
        });
      }
    }
  };
  ...
  return (
    ...
    >
      ...
      <form
        className="answer"
        onSubmit={(e) =>
          props.onSbmtHandler(e, answeredQuiz, props.activeQuestion)
        }
      >
        <div className="choice">
          {quiz.options.map((option) => {
            return (
              <div key={`${quiz.questionId}${option[0]}`}>
                <input
                  ...
                  id={option[0]}
                  checked={
                    answeredQuiz.userAnswer.includes(+option[0])
                      ? "checked"
                      : ""
                  }
        ...
        <div className="btns">
          {quiz.questionId === 1 ? null : (
            <button
              ...
              key="back"
              onClick={(e) =>
                props.onBackClickHandler(e, answeredQuiz, props.activeQuestion)
              }
            >
          ...
          {quiz.questionId === props.lastQuestion ? null : (
            <button
              ...
              key="next"
              onClick={(e) =>
                props.onNextClickHandler(e, answeredQuiz, props.activeQuestion)
              }
            >
              <span>next</span>
